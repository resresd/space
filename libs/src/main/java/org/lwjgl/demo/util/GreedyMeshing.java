...
public class GreedyMeshing {
	public static class Face implements Boundable<Face> {
		...
		public int lod() {
			return Math.min(32 - numberOfLeadingZeros(u0 ^ u1), 32 - numberOfLeadingZeros(v0 ^ v1));
		}
		...
		private Face splitLeftZ(int splitAxis, int splitPos) {
			switch (splitAxis) {
			case 0:
				return new Face(u0, v0, splitPos, v1, p, s, v, tx, ty);
			case 1:
				return new Face(u0, v0, u1, splitPos, p, s, v, tx, ty);
			default:
				throw new IllegalArgumentException();
			}
		}
		...
		private Face splitRightZ(int splitAxis, int splitPos) {
			switch (splitAxis) {
			case 0:
				return new Face(splitPos, v0, u1, v1, p, s, v, tx + (splitPos - u0), ty);
			case 1:
				return new Face(u0, splitPos, u1, v1, p, s, v, tx, ty + (splitPos - v0));
			default:
				throw new IllegalArgumentException();
			}
		}
	...
	private static int[] computeNeighborConfigs() {
		...
		for (int i = 0; i < 256; i++) {
			boolean cxny = (i & 1 << 0) == 1 << 0, nxny = (i & 1 << 1) == 1 << 1, nxcy = (i & 1 << 2) == 1 << 2,
					nxpy = (i & 1 << 3) == 1 << 3;
			boolean cxpy = (i & 1 << 4) == 1 << 4, pxpy = (i & 1 << 5) == 1 << 5, pxcy = (i & 1 << 6) == 1 << 6,
					pxny = (i & 1 << 7) == 1 << 7;
	...
	public void mesh(byte[] vs, List<Face> faces) {
		...
		List<Face>[] fs = new List[] { new ArrayList<Face>(), new ArrayList<Face>(), new ArrayList<Face>(),
				new ArrayList<Face>(), new ArrayList<Face>(), new ArrayList<Face>() };
		mesh(vs, fs);
		for (int i = 0; i < 6; i++)
			faces.addAll(fs[i]);
	}

	public void mesh(byte[] vss, List<Face>[] faces) {
		this.vs = vss;
		meshX(faces);
		meshY(faces);
		meshZ(faces);
